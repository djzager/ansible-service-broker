...
POD_NAME="$(oc get po -o jsonpath='{.items[?(@.metadata.labels.service=="'${DEPLOYMENT_NAME}'")].metadata.name}' -n ${PROJECT})"
until oc get po ${POD_NAME} -o jsonpath='{.status.containerStatuses[0].ready}' -n ${PROJECT} | grep "true" >/dev/null 2>&1 ; do sleep 1; done
